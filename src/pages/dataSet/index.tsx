...
export default () => {
  ...
  return (
    <>
      <div style={{fontSize: '20px', fontWeight: 'bold'}}>数据集列表</div>
          ...
          >
            ...
            {taskTypeId === 1 && (
              <ModalForm<{
                ...
                submitter={false}
              >
                <div style={{color: "red", fontSize: "medium"}}>图片数据</div>
                <div>
                  zip压缩文件内应包含<b>test文件夹,train文件夹,test.txt文件,train.txt文件</b>，如下
                </div>
                <Image src={datasetForm1}></Image>
                <div>txt文件应说明数据集文件与标签的对应关系，<b>每一行对应文件夹内的一条数据文件名</b>，以及对应的分类标签,如下</div>
            ...
            {taskTypeId === 3 && (
              <ModalForm<{
                ...
                submitter={false}
              >
                <div style={{color: "red", fontSize: "medium"}}>表格数据</div>
                <div className="row row-margin-top">
                  zip压缩文件内，<b>需同时包含训练集与测试集</b>，其中训练集的命名格式为：<b>数据集名称_train.csv</b>，测试集的命名格式为：<b>数据集名称_test.csv</b>，如下
                </div>
