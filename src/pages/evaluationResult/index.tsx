...
/*          if (maxFactor < item.distortion_factor[0])
      ...
      if (obj.evaluateType === '适应性') {
        ...
        res.noiseMethodList.unshift({'methodName': '原始情况', 'change_clean_acc': res.clean_acc});
        setAdaptData2(res);
...
{/*        <ProCard split="vertical" title="综合评测">
            ...
            request={async () => {
              ...
              Object.keys(data1).forEach((key) => {
                if(Math.floor(data1[key]) !== data1[key])
                  data1[key] = Number(data1[key]).toFixed(5) ;
        ...
        {evaluateType === "ADAPT" && (
        <ProCard  title={"适应性"} layout={"center"} split={"horizontal"}>
          <ProCard colSpan={12}  >
            <Column
            autoFit={true}
...
/*            label={{
          ...
          <ProCard colSpan={10} >
            <Space>在{adaptData2.batch_size}张测试图片上，进行了{adaptData2.noiseMethodList.length-1}种噪声测试,在少量噪声干扰下，准确率平均下降{adaptData2.cost}%</Space>
          </ProCard>
        </ProCard>)}

        {evaluateType === "ROBUST" && (
          <ProCard split={"horizontal"} title={"鲁棒性"} layout={"center"} bodyStyle={{padding: '80px'}}>
