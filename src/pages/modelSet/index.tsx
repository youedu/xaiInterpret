...
import {Button, Input, message, Select, Form, Table, Upload} from 'antd';
...
/*      filters: true,
  ...
  const onSelectChange = (record, selected, selectedRows, nativeEvent) => {
    if(selected === true){
      ...
      setSelectedRowKeys(selectedRowKeys.filter(item=>{return item!=record.id}));
    }
  }
...
/*    if(selected === true){
  ...
  return (
    <>
      <div style={{fontSize: '20px', fontWeight: 'bold'}}>模型列表</div>
...
/*      options={{
      ...
      toolBarRender={() =>[
        <ModalForm
          modalProps={{
            destroyOnClose: true,
            onCancel: () =>{setModelType(0);      setModelUpload({});}
          }}
          preserve={false}
          form={modalForm}
          formRef={modalRef}
            ...
            render: (props, dom) =>{
              ...
              >
                  <ProFormText
                    ...
                    required={true}
                  />
                  <ProFormTextArea label="模型描述" width="md" name="modelDesc" placeholder="请输入模型信息" required={true}/>
                  <ProFormSelect
                    width="sm"
                    name="modelFrame"
                    allowClear={false}
...
/*                      onChange={async (info) => {
                  ...
                  {(modelType === 0 || modelType === 1) && (
                    <Form.Item>
                      <div style={{fontSize: '15px'}}>可选关联数据集</div>
                      <ProTable
                        loading={false}
                        columns={dataColumns}
                  ...
                  {(modelType === 3 ) && (
                    <Form.Item>
                      ...
                      <div style={{fontSize: '15px'}}>可选关联黑盒模型</div>
                      <ProTable
                        loading={false}
                        columns={columns}
